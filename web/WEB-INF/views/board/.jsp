    ...
    <div class="container">
        ...
        <section>
			...
			<c:if test="${data.product_writer==loginUser }">
				<input type="button" 
				       value="수정" onclick="location.href='${contextPath}/board/qnamodifyform.jsp?product_no=${data.product_no }'">
				<input type="button" 
			           value="삭제" onclick="location.href='${contextPath}/board/delete?product_no=${data.product_no }&product_img=${data.product_img }'">
			    <c:choose>
                <c:when test="${not empty loginUser }"><input type="button" onclick="location.href='reply.jsp?bid =${view.bid }'" value="답글">  </c:when>  
                </c:choose>
			</c:if>
